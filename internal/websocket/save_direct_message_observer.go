...
func (s SaveDirectMessageObserver) OnSendMessage(
...
func (s SaveDirectMessageObserver) handleDirectMessage(
	message *inbound.InboundSendDirectMessage,
	messageId chat_direct_entity.ChatDirectId,
	userId user_entity.UserId,
) {
	err := s.deps.SaveDirectMessageUseCase.Execute(
		context.Background(),
		&chat_direct_entity.ChatDirect{
			Id:        messageId,
			SenderId:  userId,
			User1Id:   userId,
