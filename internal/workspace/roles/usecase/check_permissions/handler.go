...
func NewCheckPermissionsHandler(checkPermissionUseCase *permissions.CheckPermissionUseCase) *CheckPermissionsHandler {
...
func (h *CheckPermissionsHandler) Handle(c *gin.Context) {
	...
	hasPermission, err := h.checkPermissionUseCase.Execute(c, workspaceMember.Id, entity2.WorkspaceId(workspaceId), request.Permissions)
	if err != nil {
