...
func (m MongoWorkspaceMemberRepository) unsafeAddMember(ctx context.Context, member *entity2.WorkspaceMember) error {
	member.Id = entity2.WorkspaceMemberId(bson.NewObjectID().Hex())
	...
	_, err = m.deps.Client.Client.Database(databaseName).Collection(collectionName).InsertOne(ctx, mappedMember)
	if err != nil {
		return err
	}
...
func (m MongoWorkspaceMemberRepository) CountMembers(ctx context.Context, workspaceId entity.WorkspaceId) (uint, error) {
	workspaceObjectId, err := bson.ObjectIDFromHex(workspaceId.String())
	if err != nil {
		return 0, err
	}
...
func (m MongoWorkspaceMemberRepository) GetMemberByUserId(ctx context.Context, workspaceId entity.WorkspaceId, userId user_entity.UserId) (*entity2.WorkspaceMember, error) {
	workspaceObjectId, err := bson.ObjectIDFromHex(workspaceId.String())
	if err != nil {
		return nil, err
	}
	...
	var mongoWorkspaceMember MongoWorkspaceMember
	err = m.deps.Client.Client.Database(databaseName).Collection(collectionName).FindOne(ctx, bson.M{"workspace_id": workspaceObjectId, "user_id": userObjectId}).Decode(&mongoWorkspaceMember)
	if err != nil {
